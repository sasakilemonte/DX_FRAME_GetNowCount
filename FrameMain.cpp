...
//時間の取得にDXライブラリのGetNowCountを使用しています。
//時間経過に若干の誤差があり、1秒間58.8フレームで動作します。
...
//----------------------------------------------
//フレームレート設定(通常は16.6666ですが、
//返す値がINT型のためできるだけ近い数字で設定)
#define Framerate 17
//----------------------------------------------
			 ...
			 LPSTR lpCmdLine, int nCmdShow )
{
	...
	SetMainWindowText( "フレームアニメーション(DXライブラリGetNowCount使用)" ) ;
	...
	//ゲームの動作更新の最終時間です。
	//[GetNowCount()]http://homepage2.nifty.com/natupaji/DxLib/function/dxfunc_other.html#R7N1
	...
	for(;;)
	{
		...
		{
			loop_count=looptime/Framerate;	//ループ回数を求める
			int amaritime=looptime-(loop_count*Framerate);	//ループ回数の余り時間を求める
